...
ビルド時に `NEXT_PUBLIC_CURRENT_USER_ROLE=bank_staff` のように指定して初期ロールを切り替えることもできます。
...
- Supabase / SQLite / Google Sheets / CSV / Apps Script などへの保存先移行（後回し）
- インボイス対応（後回し）

## Phase 3 実装済み

- 請求書プレビュー画面からのPDF出力（ブラウザの印刷/PDF保存）
- Gmail作成画面の下書き相当リンク生成
- LINE送信用PNG画像出力
- 権限定義の型付け、ロール表示名、環境変数による初期ロール切り替え
- 個別売上調整入力のローカル永続化
- `invoice_summaries` への請求発行/送付/入金/集金ステータス保存
- 請求一覧でのステータス更新UI
...
- Phase 3: 請求書プレビューの出力ボタンはPDF保存、Gmail作成画面、LINE用PNG生成に接続済みです。
